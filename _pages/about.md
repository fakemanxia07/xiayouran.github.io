---
layout: about
title: About
permalink: /
nav: true
nav_order: 1
subtitle: Math major at <a href='https://ustc.edu.cn/' target='_blank'>USTC</a>..

profile:
  align: right
  image: prof_pic.jpg # 稍后我们需要替换这张图片
  image_circular: false 
  more_info: >
    <p>School of the Gifted Young</p>
    <p>USTC, Hefei, Anhui</p>
    <p>xiayouran@mail.ustc.edu.cn</p>

selected_papers: false # 目前还没有发表论文的话，先改成 false 隐藏起来
social: true 

announcements:
  enabled: false # 如果暂时不想写新闻动态，改成 false 让主页更清爽

latest_posts:
  enabled: false # 博客文章同理，准备好内容前可以先隐藏
---

Hello! I am Youran Xia, a 19-year-old freshmanyear student in University of Science and Technology of China. I am currently studying in the School of the Gifted Young, and I am looking foward to study mathmatics. You can contact me at xiayouran@mail.ustc.edu.cn or at fakemanxia@outlook.com. Hope we can establish good relationship!
